...
The system SHALL provide `sys init` to initialize a repo-local sys-orchestrated monorepo.

#### Scenario: Initialize empty repository
- **WHEN** a user runs `sys init` at the repository root
- **THEN** the system creates `.sys-orchestrator/`, scaffolds `/system`, records the phase as `design`, and prints the next available commands
