...
This repository is a new sys-orchestrator project with OpenSpec initialized and no application code yet. The product being introduced is a Go CLI named `sys` that governs an agent-native monorepo workflow:

- During design phase, agents use `sys explore` and `sys capture` semantics to finalize decisions directly into `/system`; OpenSpec is intentionally not used for design.
...
**Goals:**
- Implement a Go CLI named `sys`.
- Scaffold `.sys-orchestrator/` and the canonical `/system` tree.
- Track repo-local phase, freeze state, capture sessions, agent installs, and validation state.
- Provide `sys status` as the primary terminal dashboard, including JSON output for agents.
...
- Install Codex-native skills with `sys agent install codex`.
- Generate minimal Cursor and Claude Code instruction files with `sys agent install cursor` and `sys agent install claude`.
...
The CLI will be a normal Go module with a `cmd/sys` entrypoint and internal packages for commands, state, scaffolding, validation, agents, and OpenSpec execution. v1 should prefer the Go standard library for predictable bootstrapping in a new repo.
...
### Store machine state under `.sys-orchestrator/`

Repo-local machine state will live under `.sys-orchestrator/`, with JSON files for v1:
...
`sys explore` and Codex `sys-explore` guide design conversations using the current `/system` state. `sys capture` and Codex `sys-capture` guide the agent to update `/system` and add a decision record when a decision is finalized. In design phase, this is direct and reversible through normal file history.
...
After `sys design freeze`, controlled or frozen `/system` files require `sys design-change` semantics. This command creates a heavier mutation path with rationale, impacted files, and build-change impact. Normal `sys capture` is blocked in build phase.
...
`sys agent install codex` will install project-local skills:
...
### Implement `sys status` as the dashboard command

`sys status` will be the primary human dashboard. `sys status --json` will be the machine-readable interface for skills and scripts. `sys status --watch` will refresh the dashboard at an interval.

v1 can use ANSI terminal output and clear sections rather than a full TUI framework.

Alternatives considered:
- Separate `sys dashboard` command: discoverable, but it splits the status concept. It can be added later as an alias if useful.

## Risks / Trade-offs

- CLI cannot read a Codex conversation by itself -> Codex `sys-capture` skill will perform the actual extraction/editing, while the CLI supplies structure, status, templates, and validation.
- JSON state is less pleasant than YAML -> keep JSON hidden under `.sys-orchestrator/` and reserve human-facing truth for `/system`.
...
- Freeze enforcement based on recorded hashes can miss intent -> require `sys validate` and clear status warnings; deeper git integration can come later.
- OpenSpec and Superpowers are external tools/instructions -> `sys` should verify presence where possible and print exact next actions instead of pretending to replace them.
...
2. Add repo discovery and `.sys-orchestrator` state.
...
Rollback is deleting generated artifacts from the working tree before adoption. Once users run `sys init` in real projects, they should rely on git history for reversal.
