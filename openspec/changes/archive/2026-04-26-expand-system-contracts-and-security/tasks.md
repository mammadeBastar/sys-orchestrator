...
- [x] 1.6 Update `sys explore` and `sys capture` output so design guidance names contracts, contract conventions, contract errors, and security as possible target areas.

## 2. Agent Instruction Templates

- [x] 2.1 Update `sys-explore` to include security in design exploration, role guidance, and validation checks.
- [x] 2.2 Update `sys-capture` to route decisions about conventions, errors, and security posture to the new owning files.
- [x] 2.3 Update `sys-apply` to include security context when frontend or backend implementation depends on security invariants.
- [x] 2.4 Update `sys-design-change` to include `system/security/` when build-phase foundation changes affect security truth.
...
- [x] 4.3 Update freeze tests to verify build-phase mutations to the new controlled files require `sys design-change`.
