...
4. Read `.sys-orchestrator/allowlists.json` when role access is unclear.
...
- If exploration reveals that build-phase foundation truth must change, stop and direct the user to `sys design-change`.
...
7. When decisions are finalized, suggest `sys-capture` and identify the likely target files.
...
- Stop if the decision would mutate frozen or controlled build-phase truth; use `sys design-change`.
